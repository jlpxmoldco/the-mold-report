...
def call_claude(system_prompt, user_prompt, max_tokens=2000):
    ...
    date_prefix = f"Today's date is {today_str}. Use this as your reference for what is current.\n\n"
    system_prompt_with_date = date_prefix + system_prompt
...
def invention_guard(article):
    ...
    # Auto-pass articles from trusted, verified API sources
    # These were fetched directly from the source API, not generated by AI
    source_url = article.get('sourceUrl', '')
    if 'pubmed.ncbi.nlm.nih.gov' in source_url:
        print(f"    ✓ Auto-pass: trusted source (PubMed API)")
        article['_invention_pass'] = True
        article['_invention_check'] = {"real": True, "confidence": 1.0, "concerns": [], "note": "Auto-passed: fetched directly from PubMed API"}
        return article
...
def deploy_to_github():
    ...
    try:
        ...
        files_to_push = ["index.html", "articles.json", "editorial_pipeline.py", "scraper.py", "README.md", ".gitignore", "about.html", "generate_newsletter.py", "rewrite_headlines.py"]
